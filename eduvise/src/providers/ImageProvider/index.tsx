...
import { getImageAction, getAllImagesAction } from './action';
...
const ImageProvider: React.FC<ImageProviderProps> = ({ children }) => {
    ...
    const getImage = async (id: string): Promise<string> => {
      try {
        const response = await axios.get(`https://localhost:44311/File/DownLoadFile?id=${id}`);
       
        dispatch(getAllImagesAction(response.data.result));
